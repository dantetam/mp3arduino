...
char trackName[] = "track1.mp3   ";
int trackNumber = 1;
boolean requestEnd = false;
boolean requestVolUp = false;
boolean requestVolDown = false;
boolean debounce = false;
boolean volDebounce = false;
int vol = 80;
...
void setup() {
  ...
  pinMode(10, OUTPUT);       
  if (!card.init(SPI_FULL_SPEED))  Serial.println("Error: Card init"); 
  if (!volume.init(&card)) Serial.println("Error: Volume ini"); 
  if (!root.openRoot(&volume)) Serial.println("Error: Opening root"); 
...
void loop(){
  if (trackNumber >= 10)
    sprintf(trackName, "track%02d.mp3", trackNumber); //Splice the new file number into this file name
  else if (trackNumber >= 100)
    sprintf(trackName, "track%03d.mp3", trackNumber); //Splice the new file number into this file name
  else
    sprintf(trackName, "track%d.mp3", trackNumber); //Splice the new file number into this file name
  ...
  trackNumber = TrueRandom.random(1,70); //When we loop, advance to next track!
}
...
void playMP3(char* fileName) {
  ...
  while(1) {
    while(!digitalRead(MP3_DREQ)) { 
      if (digitalRead(5) && !debounce)
      {
        debounce = true;
        requestEnd = true;
      }
      else if (digitalRead(10) && !debounce)
      {
        //volDebounce = true;
        //requestVolUp = true; 
      }
      else if (digitalRead(13) && !debounce)
      {
        //volDebounce = true;
        //requestVolDown = true; 
      }
